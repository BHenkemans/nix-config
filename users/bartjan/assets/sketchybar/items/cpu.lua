...
-- Each metric is its own sketchybar item so that "all" mode can colour each
-- one independently. Items are added in display-right-to-left order so the
-- visible ordering reads cpu | temp | mem | disk | net (left to right).
...
-- Hidden driver item; always ticks and runs the exec calls. Visible items use
-- updates="when_shown" via default.lua so they wouldn't tick when hidden.
...
driver:subscribe({ "routine", "forced" }, function()
  sbar.exec(
    ...
    function(out) current.cpu = tonumber(out) or 0; render() end
  )
  sbar.exec(
    [[memory_pressure | awk '/free percentage/ {gsub("%","",$NF); printf "%d", 100 - $NF}']],
    function(out) current.mem = tonumber(out) or 0; render() end
  )
  sbar.exec(
    [[macmon pipe -s 1 2>/dev/null | jq -r '.temp.cpu_temp_avg | round']],
    function(out) current.temp = tonumber(out) or 0; render() end
  )
  sbar.exec(
    -- /System/Volumes/Data is the read-write APFS volume where user data lives;
    -- / is the read-only System volume and only ~4% on a typical install.
    [[df -k /System/Volumes/Data | awk 'NR==2 {gsub("%","",$5); print $5}']],
    function(out) current.disk = tonumber(out) or 0; render() end
  )
  ...
  sbar.exec(
    ...
    [[[ -z "$IFACE" ] && exit 0; ]] ..
    [[netstat -bn -I "$IFACE" | awk 'NR==2 {print $7, $10}']],
    function(out)
